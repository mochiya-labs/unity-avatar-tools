...
For `.vrm` files, animation remains the responsibility of the site's existing `three-vrm` flow. The exporter keeps the standard VRM humanoid, expressions, constraints, and spring-bone data in the file; the Mochiya extension only changes how marked materials are rendered.

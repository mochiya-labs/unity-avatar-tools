...
For a `.vrm` output, the same binary GLB container also includes the standard VRM 1.0 extensions emitted by UniVRM. The custom material extension does not replace `VRMC_vrm` or change the existing Mochiya website's VRM animation path.
